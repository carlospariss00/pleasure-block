...
import { generateInitialPieces, generateRandomPiece, SIZE_BONUS } from './pieces';
import type { PieceShape, GridCoordinate } from './types';
  ...
  (set, get) => ({
    ...
    placePiece: (pieceIndex, startX, startY) => {
      ...
      if (newPieces.every(p => p === null)) {
        newPieces = [
          generateRandomPiece(state.level, newBoard),
          generateRandomPiece(state.level, newBoard),
          generateRandomPiece(state.level, newBoard)
        ];
      }

      const baseScore = piece.coords.length * POINTS_PER_BLOCK;
      const bonusMultiplier = SIZE_BONUS[piece.coords.length] || 1.0;
      const addedScore = Math.floor(baseScore * bonusMultiplier);
    ...
    clearLines: () => {
      ...
      if (toClear.size > 0) {
        ...
        const fullClearBonus = isFullClear ? 1000 : 0;
        const finalScore = newScore + fullClearBonus;

        set({ 
          board: newBoard, 
          score: finalScore,
          highScore: { ...state.highScore, [gameMode]: Math.max(finalScore, state.highScore[gameMode]) },
